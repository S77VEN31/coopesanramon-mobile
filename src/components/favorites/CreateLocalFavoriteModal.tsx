...
import { TwoFactorStep, LocalAccountSearchStep, FavoriteDataStep } from '@/components/wizard/steps';
...
export default function CreateLocalFavoriteModal() {
  ...
  const searchedIbanRef = useRef<string | null>(null);
  ...
  const handleSearch = async () => {
    ...
    if (searchedIbanRef.current === unmaskedIban) return;
    searchedIbanRef.current = unmaskedIban;
  ...
  const handleWizardComplete = async () => {
    ...
    if (isValid) {
      await handleCreateAccount(currentChallenge?.idDesafioPublico || null);
    } else {
      setIsProcessing(false);
    }
  };

  const handleCreateAccount = async (idDesafio: string | null) => {
    try {
      await createFavoriteAccount({
        numeroCuenta: unmaskedIban,
        email: email || null,
  ...
  useEffect(() => {
    if (isCreateModalOpen && !currentChallenge && !isCreatingChallenge && !has2FAStepBeenInitialized && destinationAccount && alias !== undefined) {
      setHas2FAStepBeenInitialized(true);
      createdesafio(TipoOperacion.CreacionCuentaFavoritaInterna).then((challenge) => {
        if (challenge && challenge.tiempoExpiracionSegundos != null && challenge.tiempoExpiracionSegundos > 0) {
          startCountdown();
        }
      });
    }
  }, [isCreateModalOpen, currentChallenge, isCreatingChallenge, has2FAStepBeenInitialized, destinationAccount, alias, createdesafio, startCountdown]);
      ...
      component: (
        <View style={styles.stepContent}>
          <LocalAccountSearchStep
            ...
            validatedAccount={destinationAccount}
            error={destinationError}
            searchLabel={FAVORITE_TEXTS.SEARCH_IBAN_LABEL}
          />
        </View>
      ),
  ...
  ], [iban, unmaskedIban, alias, email, telefono, montoMaximo, destinationAccount, destinationError, isLoadingDestination, currentChallenge, isCreatingChallenge, isValidating, isExecutingOperation, validationError, operationError, remainingAttempts, timeRemaining, otpCode, emailCode, operationComplete, isProcessing, colorScheme, textColor, secondaryTextColor, borderColor]);
      ...
      <View style={styles.modalBackdrop}>
        ...
        <View style={[styles.modalContent, { backgroundColor }]}>
          <View style={[styles.modalHeader, { borderBottomColor: borderColor }]}>
            <Text style={[styles.modalTitle, { color: colorScheme === 'dark' ? '#ffffff' : '#a61612' }]}>{FAVORITE_TEXTS.CREATE_LOCAL_TITLE}</Text>
