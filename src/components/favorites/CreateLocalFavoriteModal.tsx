...
export default function CreateLocalFavoriteModal() {
  ...
  const handleSave = () => {
    open2FAModal(TipoOperacion.CreacionCuentaFavoritaInterna, async (idDesafio) => {
          ...
          <View style={styles.fieldsContainer}>
            ...
            {destinationAccount && (
              <View style={[styles.accountCard, { borderColor }]}>
                ...
                <Text style={[styles.accountValue, { color: textColor }]}>{destinationAccount.titular || '-'}</Text>
                <Text style={[styles.accountLabel, { color: secondaryTextColor }]}>IBAN</Text>
                <Text style={[styles.accountValue, { color: textColor }]}>{destinationAccount.cuentaIBAN || '-'}</Text>
                <Text style={[styles.accountLabel, { color: secondaryTextColor }]}>Moneda</Text>
                <Text style={[styles.accountValue, { color: textColor }]}>{destinationAccount.codigoMoneda || '-'}</Text>
              </View>
            )}
          </View>
        </ScrollView>
      ...
      <Pressable style={styles.modalBackdrop} onPress={handleClose}>
        <Pressable style={[styles.modalContent, { backgroundColor }]} onPress={(e) => e.stopPropagation()}>
          <View style={[styles.modalHeader, { borderBottomColor: borderColor }]}>
            <Text style={[styles.modalTitle, { color: '#a61612' }]}>{FAVORITE_TEXTS.CREATE_LOCAL_TITLE}</Text>
